...
inline void bind_basic_types(sol::state &lua_state)
{
    ...
    sol::usertype<yorcvs::Vec2<bool>> vec2b =
        lua_state.new_usertype<yorcvs::Vec2<bool>>("vec2b", "x", &yorcvs::Vec2<bool>::x, "y", &yorcvs::Vec2<bool>::y);
...
inline void bind_map_functions(sol::state& lua_state)
{
    ...
    lua_state["Map"]["load_entity"] = [](yorcvs::Map& map,const size_t entityID, const std::string path) { // creates a new entity and assigns components from the file
